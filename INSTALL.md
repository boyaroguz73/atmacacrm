...
- `WAHA_API_KEY` güçlü değer; backend `.env` ile aynı; `WAHA_NO_API_KEY=false` önerilir.
- **Dashboard (WAHA Plus):** kök `.env` içinde `WAHA_DASHBOARD_USERNAME`, `WAHA_DASHBOARD_PASSWORD` ve `WAHA_DASHBOARD_NO_PASSWORD=false` ayarlayın ([Dashboard yapılandırması](https://waha.devlike.pro/docs/how-to/dashboard/)). Ardından `docker compose up -d waha` (veya tüm stack).
- Paneli mümkünse yalnızca VPN / iç IP’den açın; dışarıda mutlaka kullanıcı + şifre kullanın.
